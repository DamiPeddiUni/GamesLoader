<template>
	<div class="container">
		<div class="background" :style="{'background-image': 'url(' + (processedGames.length > 0 ? processedGames[selectedGame].background : ``) + ')'}"></div>
		<Header />
		<div class="app-container">
			<Loader v-if="processedGames.length != games.length"/>
			<div class="games-container" v-else>
				<Game :id="processedGames.game_id" v-for="(game, index) in processedGames" :key="index" :game="game" :class="{'selected' : processedGames[selectedGame].game_id == game.game_id}"/>
			</div>
			<div class="menu-container" v-if="showingMenu">
				<div class="menu-background"></div>
				<div class="menu-box">
					<div class="menu-options-container">
						<MenuOption v-for="(option, index) in options" :key="index" :optionLabel="option.label" :class="{'selected' : options[selectedOption] == option}"/>
					</div>
				</div>
			</div>
		</div>
	...
	methods: {
		setupGamepad(){
			...
			function gamepadUpPressed(){
				if (self.showingMenu){
					self.selectedOption = self.selectedOption >= 1 ? (self.selectedOption - 1) : (self.options.length - 1)
				}
				allowUp = false
				setTimeout(() => {
					allowUp = true;
			...
			function gamepadDownPressed(){
				if (self.showingMenu){
					self.selectedOption = self.selectedOption < self.options.length - 1 ? self.selectedOption + 1 : 0
				}
				allowDown = false
				setTimeout(() => {
					allowDown = true;
			...
			function gamepadLeftPressed(){
				if (!self.showingMenu){
					self.selectedGame = self.selectedGame >= 1 ? (self.selectedGame - 1) : (self.games.length - 1)
					self.onGameSelected(self.games[self.selectedGame].game_id)
				}
			...
			function gamepadRightPressed(){
				if (!self.showingMenu){
					self.selectedGame = self.selectedGame < self.games.length - 1 ? self.selectedGame + 1 : 0
					self.onGameSelected(self.games[self.selectedGame].game_id)
				}
			...
			function gamepadAPressed(){
				if (self.showingMenu){
					let option = self.options[self.selectedOption]
					option.action()
				}else{
					console.log("seleziono gioco " + self.processedGames[self.selectedGame].name)
				}
				allowA = false
				setTimeout(() => {
					allowA = true;
